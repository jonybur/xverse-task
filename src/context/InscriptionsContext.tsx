...
export const InscriptionsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [cachedInscriptions, setCachedInscriptionsState] = useState<
    InscriptionsContextType['cachedInscriptions']
  >({});

  const setCachedInscriptions: InscriptionsContextType['setCachedInscriptions'] = (
    address,
    data
  ) => {
